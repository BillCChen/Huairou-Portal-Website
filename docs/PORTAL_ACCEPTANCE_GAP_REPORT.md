...
V1 中首页、新闻、案例、关于我们、登录、注册、审核、领导团队已有基础实现。P1-B/P1-C 已完成邮箱密码重置后端与前端基础；P1-D 已完成机构用户管理员创建、审核驳回、禁用/启用、角色赋权和用户生命周期 smoke；P1-E 已完成 V1 内容 CMS 验收闭环和 `scripts/smoke_v1_content_backend.sh`。剩余主要缺口包括：P1 汇总验收文档、权限矩阵扩展测试、测试报告、性能压测、安全扫描、生产部署验收。
...
| 邮箱密码重置 | 后端与前端基础已实现 | `/auth/password-reset/request`, `/auth/password-reset/confirm`, `password_reset_tokens`, `/forgot-password`, `/password-reset/confirm` | 仍需真实 SMTP UAT、full-link UAT | P1/P2 |
| 机构用户管理员分配 | 已实现基础闭环 | `/admin/users`, `/admin/users/{id}/role`, `UsersView.vue` | 仍需更完整权限矩阵测试 | P1-F |
| 个人注册 | 基本可用且已有 lifecycle smoke | /auth/register, `scripts/smoke_user_lifecycle_backend.sh` | 更完整表单与权限矩阵仍待 P1-F | P1-F |
| 注册审核 | 已补齐 V1 状态闭环 | approve/reject/disable/enable endpoints, user lifecycle smoke | 重新提交/批量审核仍非 V1 必需 | P1-F/P2 |
| 机构概况/关于我们 | P1-E 已闭环 | Page/SiteSetting, about blocks, content smoke | 图片/文件公开 URL 仍非本阶段范围 | P1-F/P2 |
| 新闻动态 | P1-E 已闭环 | Article routes/pages/admin, content smoke | 删除/预览/附件展示仍非 V1 必需 | P1-F/P2 |
| 成功案例 | P1-E 已闭环 | Case routes/pages/admin, partner/benefits/result_blocks | 关联研究所实体化仍属 V2/P2+ | P1-F/P2 |
