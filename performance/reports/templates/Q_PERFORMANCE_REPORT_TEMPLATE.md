...
如稳定性通过但延迟未完全达标，应使用保守口径：

在 100 个并发虚拟用户的低扰动只读场景下，系统保持可用，未观察到 5xx，请求失败率满足稳定性目标；但某一页面或接口的严格 p95 延迟门槛未完全通过。该结果应表述为“稳定性通过，延迟仍有优化空间”，不能表述为“100 并发全部指标通过”。
...
- 100 并发完全无压力。
- 100 VU 全部指标通过，除非所有阈值都通过。
- 系统已经完成性能验收，除非验收范围和门槛均已明确通过。
- 可承诺 100 QPS。
